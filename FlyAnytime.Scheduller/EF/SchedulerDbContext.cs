...
namespace FlyAnytime.Scheduler.EF
{
    public class SchedulerDbContext : BaseEfContext<SchedulerDbContext>
    {
        ...
        public async Task CreateDbIfNotExists()
        {
            ...
            var needToCreateQuartz = false;

            var conn = Database.GetDbConnection();
            using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "SELECT COUNT(1) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'QRTZ_CALENDARS'";
                    await conn.OpenAsync();
                    var countOfTables = Convert.ToInt32(cmd.ExecuteScalar());

                    needToCreateQuartz = countOfTables == 0;
                }
            
            if (needToCreateQuartz)
                await CreateQuartzTables();
        }
        ...
        private async Task CreateQuartzTables()
        {
            var sql = 
...
GO";
            ...
            var parts = sql.Split("GO", StringSplitOptions.RemoveEmptyEntries);

            foreach (var p in parts)
                await Database.ExecuteSqlRawAsync(p);            
        }

        protected override async Task DropDb()
        {
            await base.DropDb();
        }
    }
